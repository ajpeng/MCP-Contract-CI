import type { CapabilityManifest, ChangeKind, ContractChange, DiffResult, Json, JsonSchema, ToolContract } from "./types.js";
...
function isNarrower(oldSchema: JsonSchema, newSchema: JsonSchema): boolean {
  const oldTypes = types(oldSchema);
  const newTypes = types(newSchema);
  if (oldTypes && newTypes && oldTypes.some((type) => !newTypes.includes(type))) return true;
  const oldEnum = oldSchema.enum;
  const newEnum = newSchema.enum;
  if (!oldEnum && (newEnum || newSchema.const !== undefined)) return true;
  if (oldEnum && newEnum && oldEnum.some((item) => !newEnum.some((next) => stable(item) === stable(next)))) return true;
  if (oldSchema.enum && newSchema.const && !oldSchema.enum.some((item) => stable(item) === stable(newSchema.const))) return true;
  if (oldSchema.additionalProperties !== false && newSchema.additionalProperties === false) return true;
  return false;
}

function add(changes: ContractChange[], kind: ChangeKind, subject: string, message: string, path?: string): void {
  changes.push({ kind, subject, path, message, breaking: true });
}

function compareInputSchema(tool: ToolContract, previous: JsonSchema, candidate: JsonSchema, changes: ContractChange[], path = "input"): void {
  ...
  for (const name of newRequired) {
    if (!oldProperties[name]) add(changes, "input-field-now-required", tool.name, `${tool.name} adds required input field '${name}'`, `${path}.${name}`);
  }
}

function compareOutputSchema(tool: ToolContract, previous: JsonSchema, candidate: JsonSchema, changes: ContractChange[], path = "output"): void {
  if (isNarrower(previous, candidate)) add(changes, "output-schema-changed", tool.name, `${tool.name} narrows output values at ${path}`, path);
...
function likelyRenamed(previous: ToolContract, additions: ToolContract[]): string | undefined {
...
export function diffManifests(baseline: CapabilityManifest, candidate: CapabilityManifest): DiffResult {
  ...
  for (const previous of baseline.tools) {
    ...
    if (!next) {
      ...
      if (renamed) add(changes, "tool-renamed", previous.name, `Tool '${previous.name}' was renamed to '${renamed}'`);
      else add(changes, "tool-removed", previous.name, `Tool '${previous.name}' was removed`);
      continue;
    }
    compareInputSchema(previous, previous.inputSchema, next.inputSchema, changes);
    if (previous.outputSchema && next.outputSchema) compareOutputSchema(previous, previous.outputSchema, next.outputSchema, changes);
    if (previous.outputSchema && !next.outputSchema) add(changes, "output-schema-changed", previous.name, `${previous.name} no longer declares an output schema`, "output");
  }

  const resources = new Set((candidate.resources ?? []).map((resource) => resource.uri));
  for (const resource of baseline.resources ?? []) {
    if (!resources.has(resource.uri)) add(changes, "resource-removed", resource.uri, `Resource '${resource.uri}' was removed`);
  }
  const prompts = new Set((candidate.prompts ?? []).map((prompt) => prompt.name));
  for (const prompt of baseline.prompts ?? []) {
    if (!prompts.has(prompt.name)) add(changes, "prompt-removed", prompt.name, `Prompt '${prompt.name}' was removed`);
  }
...
export function formatText(result: DiffResult): string {
  if (result.summary.breaking === 0) return "No breaking MCP contract changes detected.";
