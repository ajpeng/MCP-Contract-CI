...
async function loadTrace(path) {
    ...
    if (!value || typeof value !== "object" || value.version !== 1 || !Array.isArray(value.calls)) {
...
async function main(args) {
    ...
    if (command === "diff") {
        ...
        if (!baselinePath || !candidatePath)
            throw new Error("diff needs a baseline and candidate manifest");
        ...
        if (option(rest, "--fail-on") === "breaking" && result.summary.breaking > 0)
            process.exitCode = 1;
        return;
    }
    if (command === "replay") {
        ...
        if (!tracePath || !manifestPath)
            throw new Error("replay needs a trace and manifest");
        ...
        if ((option(rest, "--format") ?? "text") === "json")
            console.log(JSON.stringify(result, null, 2));
        else {
            for (const call of result.calls)
                console.log(`${call.passed ? "PASS" : "FAIL"} ${call.tool}${call.reason ? `: ${call.reason}` : ""}`);
    ...
    if (command === "manifest" && rest[0] === "publish") {
        ...
        if (!manifestPath || !out)
            throw new Error("manifest publish needs a manifest and --out path");
