...
test("rejects saved arguments that violate const constraints", () => {
  const constrainedManifest: CapabilityManifest = {
    ...manifest,
    tools: [{
      name: "search",
      inputSchema: {
  ...
  const trace: TraceFile = { version: 1, calls: [{ tool: "search", arguments: { query: "other" } }] };
  const result = replayTrace(trace, constrainedManifest);
  assert.equal(result.passed, false);
  assert.equal(result.calls[0]?.reason, "Saved arguments no longer satisfy the input schema");
});
